...
mydb = mysql.connector.connect(host="projectdb.coe4avf23rbf.ap-south-1.rds.amazonaws.com", user="bankdbgroup",
                               passwd="bdbcse202", database="test1")
...
def Transactions():
    if 'usr' in session:
        ...
        return render_template("Transactions.html", bank=BankTransactionsdetails, credit=CreditTransactionsdetails,
                               ...
                               direct=DirectTransactionsdetails)
...
def Support():
    # if type=="----Complaint related to issues----":
    #     return render_template("Support.html")
    # else:
    #     return redirect()
    issues = {"Loans", "Debit Cards", "Credit Cards", "Fixed Deposits", "Recurring Deposits", "Payment issues",
              "Account issues", "Other"}
    select = request.form.get('form_select')
    # for i in issues:
    #     if select == issues[i]:
    ...
    return render_template("Support.html", select=select)
